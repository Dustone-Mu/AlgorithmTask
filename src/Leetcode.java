...
public class Leetcode {
    public int numRescueBoats(int[] people, int limit) {
        ...
        if (heavy==limit) {nboats+=1;}
        return nboats;
    }
    // 268
    public static int missingNumber(int[] nums) {
        for (int i = 0; i < nums.length; i++) {  //除n外，将所有元素放到它应在的位置
    ...
    public int compareVersion(String version1, String version2) {
        ...
        for(int i = 0; i < Math.max(v1.length, v2.length); i++) {
            int v1i = (i < v1.length? Integer.valueOf(v1[i]): 0);
            int v2i = (i < v2.length? Integer.valueOf(v2[i]): 0);
            if(v1i > v2i) return 1;
    ...
    public int[] intersection(int[] nums1, int[] nums2) {
        ...
        for(int i = 0, j = 0; i < nums1.length && j < nums2.length;) {
            if(nums1[i] == nums2[j]) {
                if(!(Arrays.asList(nums).contains(nums1[i]))) {
                    nums[k] = nums1[i];
                    k++;
                }
                i++;
    ...
    public int maxSubArray(int[] nums) {
        int maxS = Arrays.stream(nums).max().getAsInt();
        for(int start = 0; start < nums.length; start++) {
            if(nums[start] <= 0) continue;
            for(int end = start+2; end <= nums.length; end++) {
                int sumSi = Arrays.stream(Arrays.copyOfRange(nums, start, end)).sum();
                if(sumSi <= 0) break;
                maxS = Math.max(maxS, sumSi);
            }
        }
        return maxS;
    }
    ...
    public int findKthLargest(int[] nums, int k) {
        ...
        while (high - low > 0) {
            int pivot = partitionReverse(nums, low, high);
            if (pivot == low) low++;
            else if (pivot == k - 1) break;
            else if (pivot > k - 1) high = pivot;
            else low = pivot;
        }
        return nums[k - 1];
    }
    private int partitionReverse(int[] arr, int low, int high){ // 快速排序逆序，加入随机
    ...
    public int canCompleteCircuit(int[] gas, int[] cost) {
        // ci > gi 则不能作为出发点
        // 若不能到达某站点，那么起始点到该站点的前一站点都不能作为出发站点
        // 优化：保存start之前站点结余
        //      回首路程，延后start，可能还到不了当前，所以如果到了最后一站，且结余大于have，就成了
        int n = gas.length;
        int gasAll = 0;
        int costAll = 0;  // 若大于gas，不能
        int i;
        for (i = 0; i < n; i++) {
            gasAll += gas[i];
            costAll += cost[i];
        ...
        if (costAll > gasAll) return -1;  // can never reach
        int gasHave = 0;
        int gasPass = 0;
        for (i = 0; i < n; i++) {
            gasHave += gas[i];
            if (gasHave < 0) {  // cant reach next
                gasPass += gasHave;
                gasHave = 0;
            }
        }
        if (gasPass+gasHave < 0) return -1;
        return i;
    }
    // 128
    public int longestConsecutive(int[] nums) {
        ...
        for (int num : nums) {
            if (map.get(num) == null) map.put(num, 1);  // 如果没key，默认为1
            if (map.get(num + 1) != null) map.put(num, map.get(num + 1) + 1);
            while (map.get(--num) != null) map.put(num, map.get(num + 1) + 1);
        }
    ...
    public boolean canConstruct(String ransomNote, String magazine) {
        ...
        for (int i = 0; i < magazine.length(); i++) {
            ...
            if (note.containsKey(c)) {
                int count = note.get(c);
                if (count > 0)  note.put(c, count-1);
            }
        }
        for (int left: note.values()) {
...
//        String v1 = "0.1";
//        String v2 = "1.1";
//        System.out.println(fun.compareVersion(v1, v2));
        // 268
//        int[] people = {9,6,4,2,3,5,7,0,1};
//        int res = Arrays.binarySearch(people, people[1]);
